...
class Sub_unit extends CI_Controller {
		 ...
		 *
		 * url : http://localhost/github/office/application/modules/admin/controllers/sub_unit.php
		...
		function index()
		{
			...
			redirect('admin/sub_unit/manage');
		}
		...
		function manage()
		{
			...
			$table = 'var_sub_unit';
			$id_field = 'vsu_id';
			$relation_id_field = 'vsu_vu_id';
			
			$parent_table = 'var_unit';
			$parent_id_field = 'vu_id';
		...
		function add()
		{
			...
			# call views
			$this->load->view('sub_unit/form_sub_unit', $data);
		}
		...
		function save()
		{
			...
			else
			{
				...
				# redirect to station list
				redirect('admin/sub_unit/manage/' . $parent_id, 'refresh');
			}
		}
		...
		function delete()
		{
			...
			$sub_unit_code = $this->uri->segment(4, 'error');
			
			# anticipate uri error
			if($sub_unit_code == 'error'){redirect('admin/unit/');}
			
			# call models
			$this->user_level->delete_sub_unit($sub_unit_code);
			
			# redirect to station table
			redirect('admin/sub_unit', 'refresh');
		}
		
	}

/* End of file sub_unit.php */
/* Location: ./application/modules/admin/controllers/sub_unit.php */	
?>