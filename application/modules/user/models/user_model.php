...
# get sub agent available ---------------------------------------------- 		
	function get_sub_agent($agent) 
	{
		//$query = "SELECT * FROM wmsi_sub_agent LEFT JOIN wmsi_agent_list ON sag_agent = id_agent  WHERE agent_name='$agent' ";
		$query = "SELECT * FROM wmsi_sub_agent";
...
# get all shift --------------------------------------------------------
	...
	{
		$query = $this->db->query('SELECT * FROM wmsi_shift ORDER BY id_shift DESC');
		return $query->result();
	}
# get all shift --------------------------------------------------------
...
# get shift --------------------------------------------------------
	...
	{
		$query = $this->db->query("SELECT * FROM wmsi_shift WHERE ((sft_start <= '$time') AND (sft_end > '$time')) OR (((sft_start <= '$time') OR (sft_end > '$time')) AND (sft_next_day = 'yes'))  ");
		$result = $query->result();
		foreach($result as $row){
			return $row->sft_shift;	
		}
	}
# get shift --------------------------------------------------------
